...
# make a new column label which is just negative pick
...
pos_le = LabelEncoder()
combined_df["Team_encoded"] = team_le.fit_transform(
    combined_df["Pre-Draft Team"])
...
pre_tested_players["Team_encoded"] = team_le.fit_transform(
    pre_tested_players["Pre-Draft Team"])
pre_tested_players["Position_encoded"] = pos_le.fit_transform(
    pre_tested_players["Pos"])

# a similar checkpoint. This is going to be our testing draft set, encoded and labeled just as the training set was.
pre_tested_players.to_csv(
    "combined_player_data_with_labels_TEST_DATA.csv", index=False)
...
for col in pre_tested_players.columns:
    pre_tested_players[col] = pd.to_numeric(
        pre_tested_players[col], errors="coerce")
...
# some renaming
merged_names_and_picks = merged_names_and_picks.rename(
    columns={"RowIndex": "Predicted Pick", "Pick": "Actual Pick"})

# Set predicted pick to 61 if it's >= 60
merged_names_and_picks["Predicted Pick"] = merged_names_and_picks["Predicted Pick"].apply(
    lambda x: 61 if x >= 60 else x)

# Set actual pick to 61 if it's 0
merged_names_and_picks["Actual Pick"] = merged_names_and_picks["Actual Pick"].apply(
    lambda x: 61 if x == 0 else x)
...
merged_names_and_picks = merged_names_and_picks[
    (merged_names_and_picks["Predicted Pick"] != 61) | (
        merged_names_and_picks["Actual Pick"] != 61)
]


# pick error
merged_names_and_picks["Error (pick distance)"] = (
    merged_names_and_picks["Predicted Pick"] - merged_names_and_picks["Actual Pick"]).abs()

# output
print(merged_names_and_picks)
print("Mean AVG pick error (for now this is a very unfavorably skewed metric):",
      merged_names_and_picks["Error (pick distance)"].mean())
