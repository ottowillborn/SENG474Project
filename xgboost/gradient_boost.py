...
def train_and_test_xgboost(data_folder: str, test_year: str):

    #create path to draft class folder, match all individual file paths using glob
    test_file_name = f"{data_folder}all_players_career_stats_{test_year}.csv"
    pattern = os.path.join(data_folder, "all_players_career_stats_*.csv")
    all_files = glob.glob(pattern)
    print(test_file_name)
    # create list dfs with dataframed draft class csvs, but exclude the test file
    files = [f for f in all_files if not f.endswith(f"all_players_career_stats_{test_year}.csv")] 
    dfs = []
    for file in files:
    ...
    pre_tested_players = pd.read_csv(test_file_name)
    ...
    results_df = pd.DataFrame({
        "Player": merged_names_and_picks["Player"],
        "Actual Pick": merged_names_and_picks["Actual Pick"],
        "Predicted Pick": merged_names_and_picks["Predicted Pick"],
        "Pick Error": merged_names_and_picks["Error (pick distance)"]
    })
    print(results_df["Pick Error"].mean())
    return results_df

def main():
    norm_eq_model = train_and_test_xgboost("../allUpdatedPlayerData/", "2025")
