...
import os
import sys
def train_and_test_normal_eq(data_folder: str, test_year: str):

    #create path to draft class folder, match all individual file paths using glob
    test_file_name = f"all_players_career_stats_{test_year}.csv"
    ...
    #Get test data
    test_path = f"{data_folder}/{test_file_name}"
    ...
    # Extract fields for final results
    player_names = test_data_sorted["Player"] if "Player" in test_data_sorted.columns else [f"Player {i+1}" for i in range(len(test_data_sorted))]
    actual_picks = test_data_sorted["Pick"]
    predicted_picks = test_data_sorted["predicted_pick"]
    pick_error = np.abs(actual_picks - predicted_picks)
    ...
    # plt.show()
def main():
    norm_eq_model = train_and_test_normal_eq("../allUpdatedPlayerData/", "2025")
